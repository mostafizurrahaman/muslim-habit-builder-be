...
import {
  activateConnectedObligatoryHabit,
  activateCustomHabit,
  activateGroupHabit,
  activateSingleHabit,
  deactivateConnectedObligatoryHabit,
...
const toggleHabit = async (user: IUser, habitId: string, isActive: boolean) => {
  ...
  if (!isActive) {
    ...
    if (childTemplates.length > 0) {
      await deactivateGroupHabit(
        userId,
        childTemplates.map((c) => c._id),
        date,
      );
      return null;
    }
...
const dedupeConnectedChildren = <T extends { userHabit?: unknown }>(children: T[]): T[] => {
  ...
  for (const child of children) {
    ...
    const templateId = nested && typeof nested === 'object' ? toIdString(nested.template) : null;
...
const getTodayHabits = async (user: IUser, category?: string) => {
  ...
  const todayDayName = moment(dateStr).format('ddd').toLowerCase() as WeekDay;
  ...
  const logMap = new Map<string, string>(existingLogs.map((l: any) => [l.userHabit?.toString(), l.status]));
...
const reorderTodayHabits = async (user: IUser, habitIds: ReorderHabitsPayload['habitIds']) => {
  const userId = user._id as Types.ObjectId;
  ...
  const allActiveHabits = await UserHabit.find({
    ...
    .select('_id parent connectedHabits displayOrder')
    .lean();
  ...
  const topLevelHabits = allActiveHabits
    .filter((h) => !nestedHabitIds.has(h._id.toString()))
    .sort((a, b) => (a.displayOrder ?? 0) - (b.displayOrder ?? 0) || a._id.toString().localeCompare(b._id.toString()));

  const topLevelIds = new Set(topLevelHabits.map((h) => h._id.toString()));

  for (const id of habitIds) {
    if (!topLevelIds.has(id)) {
      throw new BadRequestError('One or more habits cannot be reordered. Only top-level habits can be moved.');
    }
  }

  const subset = new Set(habitIds);
  const queue = [...habitIds];
  const orderedIds = topLevelHabits.map((h) => {
    const id = h._id.toString();
    return subset.has(id) ? (queue.shift() as string) : id;
  });
  ...
  return { habitIds: orderedIds };
};

const reorderSubHabits = async (
  user: IUser,
  parentHabitId: string,
  subHabitIds: ReorderSubHabitsPayload['subHabitIds'],
) => {
  const userId = user._id as Types.ObjectId;
  ...
  if (!parentHabit) {
    throw new NotFoundError('Parent habit not found or inactive');
  }

  const existingConnectedHabits = parentHabit.connectedHabits ?? [];
  if (existingConnectedHabits.length === 0) {
    throw new BadRequestError('This parent habit has no connected sub-habits to reorder');
  }

  const existingIdSet = new Set<string>();
  for (const c of existingConnectedHabits) {
    const id = toIdString(c.userHabit);
    if (id) existingIdSet.add(id);
  }

  for (const id of subHabitIds) {
    if (!existingIdSet.has(id)) {
      throw new BadRequestError(`Habit with ID '${id}' is not a connected sub-habit of this parent`);
    }
  }

  const subset = new Set(subHabitIds);
  const queue = [...subHabitIds];

  const sortedExisting = [...existingConnectedHabits].sort(
    (a, b) =>
      (a.order ?? 0) - (b.order ?? 0) ||
      (toIdString(a.userHabit) ?? '').localeCompare(toIdString(b.userHabit) ?? ''),
  );

  const updatedConnectedHabits = sortedExisting.map((c) => {
    const id = toIdString(c.userHabit) as string;
    const targetId = subset.has(id) ? (queue.shift() as string) : id;
    return {
      userHabit: new Types.ObjectId(targetId),
      order: 0,
    };
  });

  updatedConnectedHabits.forEach((c, index) => {
    c.order = index + 1;
  });

  parentHabit.connectedHabits = updatedConnectedHabits;
  parentHabit.markModified('connectedHabits');
  await parentHabit.save();

  return {
    parentHabitId: parentHabit._id,
    subHabitIds: updatedConnectedHabits.map((c) => c.userHabit.toString()),
  };
};
...
const updateUserHabit = async (user: IUser, userHabitId: string, payload: EditHabitPayload) => {
  ...
  if (payload.frequency) {
    if (!habit.allowedFrequencies.includes(payload.frequency.type as any)) {
      throw new BadRequestError(`Frequency '${payload.frequency.type}' is not allowed for this habit`);
    }
  ...
  if (!effectiveIsPrayerLocked && payload.connectedPrayer !== undefined) {
    ...
    const maxOrder = refreshedConnectHabit.connectedHabits?.reduce((max, item) => (item.order > max ? item.order : max), 0) ?? 0;
  ...
  if (payload.connectedHabits && payload.connectedHabits.length > 0) {
    ...
    if (uniqueNewIds.length) {
      ...
      await UserHabit.updateMany({ _id: { $in: uniqueNewIds }, user: userId }, { $set: { parent: habit.template?._id ?? null } });
    }
    ...
    let updatedConnectedHabits = existingConnectedHabits.filter((item) => !idsToRemove.includes(item.userHabit.toString()));

    // Find the max order among remaining items
    const maxOrder = updatedConnectedHabits.reduce((max, item) => (item.order > max ? item.order : max), 0);
...
const resolveHabitDetailDisplay = (h: any) => {
  ...
  return {
    ...
    connectedPrayer: h.prayerCustomizedAt ? (h.connectedPrayer ?? null) : (template?.connectedPrayer ?? h.connectedPrayer ?? null),
  };
};
...
const getHabitDetail = async (user: IUser, userHabitId: string) => {
  ...
  const allowedPrayers: string[] = template ? (template?.allowConnectedPrayers ?? []) : (Object.values(OBLIGATORY_PRAYER) as string[]);
  const allowedFrequencies: string[] = template?.allowedFrequencies ?? habit.allowedFrequencies ?? [];

  if (template) {
    const storedFrequenciesMatch =
      JSON.stringify([...(habit.allowedFrequencies ?? [])].sort()) === JSON.stringify([...allowedFrequencies].sort());
  ...
  if (!isPrayerLocked && habit.connectedPrayer && template) {
    ...
    if (!stillAllowed) {
      await UserHabit.updateOne({ _id: habit._id }, { $set: { connectedPrayer: null, parent: null } });
  ...
  if (habit.frequency?.type && !allowedFrequencies.includes(habit.frequency.type)) {
    ...
    await UserHabit.updateOne({ _id: habit._id }, { $set: { frequency: fallbackFrequency } });
...
const searchHabitsToConnect = async (user: IUser, userHabitId: string, searchTerm?: string) => {
  const userId = user._id as Types.ObjectId;
